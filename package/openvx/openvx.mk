...
define OPENVX_INSTALL_CMD
	$(INSTALL) -D -m 0755 $(@D)/libopenvx.so.$(OPENVX_VERSION) $(1)/usr/lib/libopenvx.so.$(OPENVX_VERSION)
	ln -sf libopenvx.so.$(OPENVX_VERSION) $(1)/usr/lib/libopenvx.so
	ln -sf libopenvx.so.$(OPENVX_VERSION) $(1)/usr/lib/libopenvx.so.$(OPENVX_MAJOR_VERSION)
endef

define OPENVX_INSTALL_STAGING_CMDS
	$(call OPENVX_INSTALL_CMD,$(STAGING_DIR))
	$(INSTALL) -D -m 0644 $(@D)/openvx-extended.h $(STAGING_DIR)/usr/include/openvx_extended.h
endef
